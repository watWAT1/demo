...
with st.sidebar:
    ...
    # 更新当前页面状态
    if "🍜 美食数据仪表盘" in page_option:
        st.session_state.current_page = "美食数据仪表盘"
    elif "📄 个人简历生成器" in page_option:
        st.session_state.current_page = "个人简历生成器"
    elif "⚔️ 亚索英雄介绍" in page_option:
        st.session_state.current_page = "亚索英雄介绍"
    elif "🐒 动物园轮播相册" in page_option:
        st.session_state.current_page = "动物园轮播相册"
    elif "🎵 音乐播放器" in page_option:
...
if st.session_state.current_page == "美食数据仪表盘":
    ...
    **小贴士**: 
    ...
    with col1:
        if st.button("◀️ 上一张", use_container_width=True, on_click=prev_image):
            pass  # 导航逻辑已在on_click中处理
    ...
    with col3:
        if st.button("下一张 ▶️", use_container_width=True, on_click=next_image):
            pass  # 导航逻辑已在on_click中处理
    ...
    with col_control1:
        if st.button("⏪ 上一集", use_container_width=True, key="video_prev"):
            # 直接更新状态，Streamlit会自动重新运行
            st.session_state['ind'] = (st.session_state['ind'] - 1) % len(video_arr)
            # 不需要手动调用 st.rerun()

    with col_control2:
        if st.button("⏸️ 暂停", use_container_width=True, key="video_pause"):
            st.info("视频暂停功能需要JavaScript支持")

    with col_control3:
        if st.button("▶️ 播放", use_container_width=True, key="video_play"):
            st.success("视频正在播放")

    with col_control4:
        if st.button("🔇 静音", use_container_width=True, key="video_mute"):
            st.warning("静音功能需要JavaScript支持")
    ...
    # 使用key参数让Streamlit自动管理状态，而不是在回调中手动调用rerun
    ...
    for i in range(len(video_arr)):
        with cols[i % 3]:
            is_current = i == st.session_state['ind']
            button_label = f"▶️ 第{i + 1}集" if is_current else f"第{i + 1}集"
    ...
    video_descriptions = [
        ...
        "光头强意外发现了自己的绘画天赋，开始尝试成为一名漫画家。",
        "森林里举办了一场盛大的游园会，各种有趣的游戏等待着大家。"
    ]
